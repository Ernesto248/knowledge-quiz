...
import { DifficultyType, Question, Quiz, QuestionAmountType } from "../../types/types.d"
...
const useQuiz = ({difficulty, questionAmount}:Props) => {
    ...
    const getQuiz = async()=>{
        try{
            ...
            const filteredQuestions:Question[] = data.questions.filter((_,index)=>randomIndexes.includes(index))
